...
class Solution
{
    ...
    int knapsackRec(vector<int> value , vector<int> weight , int W, int n)
    {
        ...
        return max(value[n-1]+ knapsackRec(value, weight, W-weight[n-1] , n) , knapsackRec(value, weight, W, n-1 ));
    ...
    int knapsackMemo(vector<int> value , vector<int> weight , int W, int n)
    {
        ...
        dp[n][W] =  max(value[n-1]+ knapsackMemo(value, weight, W-weight[n-1] , n) , knapsackMemo(value, weight, W, n-1 ));
